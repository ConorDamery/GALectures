 ...
 *****************************************************************************/
/* Prefer the use of the general frustum below
...
{
    float rl = 1.0 / (proj[0].x - proj[0].y);
    float tb = 1.0 / (proj[1].x - proj[1].y);
    float nf = 1.0 / (proj[2].x - proj[2].y);

    float sx = 2.0 * proj[2].x * rl;
    float sy = 2.0 * proj[2].x * tb;
    float ox = (proj[0].x + proj[0].y) * rl;
    float oy = (proj[1].x + proj[1].y) * tb;
    float fa = (proj[2].x + proj[2].y) * nf;
    float fb = 2.0 * proj[2].x * proj[2].y * nf;
